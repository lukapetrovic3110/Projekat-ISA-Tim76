            ...
            <v-toolbar-title v-if="$refs.calendar">
              ...
              <v-chip
                ...
                v-html="
                  ...
                  this.shipAvailabilityStart +
                  ' to ' +
                  this.shipAvailabilityEnd +
                  '.'
                "
              ></v-chip>
            </v-toolbar-title>
...
export default {
  ...
  methods: {
    ...
    getAvailabilityCalendarInformation() {
      ...
      this.axios
        .get(
          "http://localhost:8091/reservationShip/availabilityCalendar/" +
            this.shipId,
          {
            headers: {
        ...
        .then((response) => {
          this.reservations = response.data.shipReservations;
          this.shipAvailabilityStart = new Date(
            response.data.availabilityStart
          ).toLocaleString();
          this.shipAvailabilityEnd = new Date(
    ...
    updateRange() {
      ...
      this.reservations.forEach((shipReservation) => {
        events.push({
          name: shipReservation.shipName,
          clientFirstName: shipReservation.clientFirstName,
          clientLastName: shipReservation.clientLastName,
          clientEmail: shipReservation.clientEmail,
          start: shipReservation.reservationStartDate,
          end: shipReservation.reservationEndDate,
          color:
            this.colors[this.selectColor(shipReservation.reservationStatus)],
          timed: true,
        });
      });
